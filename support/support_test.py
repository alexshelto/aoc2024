...
def test_next_n_points() -> None:
    ...
    bl = support.GridNav.next_n_points(
        support.XY_8Offsets.DOWN_LEFT, (6, 6), 3)
...
def test_next_n_points_generator() -> None:
    values = set()
    for point in support.GridNav.next_n_points_generator(
        support.XY_8Offsets.UP_LEFT, (6, 6), 3
    ):
        values.add(point)
    assert values == set([(5, 5), (4, 4), (3, 3)])
